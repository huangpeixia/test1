...
字典树
优点：
1.查询效率高，时间复杂度为 O(m)，其中 m 是待查询的单词长度，与字典树中数据的规模无关。
2.前缀匹配，可以高效地查找某个前缀相关的所有单词。
3.插入和删除操作复杂度为 O(m)，出现次数不影响操作时间。
4.压缩空间，节省存储空间，尤其是处理大量短字符串的场景。

缺点：
1.由于每个节点都包含了子节点信息，所以空间利用率不高，尤其是当字符串中包含大量相同前缀的时候。
2.不支持模糊匹配和查询。可以通过在字典树上做深度搜索的方式实现，但开销比较大。
3.当字符串中包含大量非 ASCII 字符的时候，会导致树的分叉过多，每个节点需要存储更多的数据。
4.由于字典树通常构造后很少改变，且存储结构比较复杂，所以建树难度比较大，需要一定的技巧和经验。


提高字典树的查询效率：
1.压缩字典树
在一些应用场景下，字典树的高度可能会非常高，导致查询效率不佳。为了解决这个问题，可以使用压缩字典树的技术，例如基于前缀树的哈弗曼编码（Huffman Coding）。Huffman 编码是一种将字符集合映射为可变长度编码的方法。基本思想是用较短的编码表示出现频率高的字符，用较长的编码表示出现频率低的字符。这个技术可以应用于字典树的优化，减少字典树节点的数量，提高查询效率。

2.使用查找表
另一种提高查询效率的方法是使用查找表。在字典树中，如果某些子树下的节点都没有被标记为单词的结束点，那么这些子树对于查询过程来说是没有意义的，我们可以将它们合并到一个 preNode 中，这个 preNode 只需要记录它们共同的前缀即可。在查询过程中，如果遇到一个 preNode，那么我们只需要在 preNode 对应的前缀基础上继续查找即可。
...
链接：https://pan.baidu.com/s/1oo6n3AWK0AAjpjhFHJ9Pdg?pwd=1234 
提取码：1234
...
链接：https://pan.baidu.com/s/1Da1mCA0TGvyPHSFIlBzENw?pwd=1234 
提取码：1234
