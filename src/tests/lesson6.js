...
const main = () => {
  ...
  assert(
    latestUTXOPool.utxos[miner] && latestUTXOPool.utxos[miner].amount == 36.49,
    'Error: miner should got right balance',
  )

  assert(
    latestUTXOPool.utxos[receiverPubKey] &&
      latestUTXOPool.utxos[receiverPubKey].amount == 1.01,
  ...
  assert(
    latestUTXOPool.utxos[receiverPubKey] &&
      latestUTXOPool.utxos[receiverPubKey].amount == 0.9,
    'Error: receiver should got right balance',
  )

  assert(
    latestUTXOPool.utxos[miner] && latestUTXOPool.utxos[miner].amount == 36.6,
    'Error: miner should got right balance',
  )
}
